...
fn new_ide_app(root string) IdeApp {
	...
	mut app := IdeApp{
		...
		path_input: os.join_path(real_root, 'form.qml')
	}
	app.sync_source()
	...
	match field {
		'property_name' {
			if !valid_identifier(value) {
				app.status = 'Name must be a V/QML identifier.'
				return false
			}
		...
		'property_event' {
			if value.len > 0 && !valid_identifier(value) {
				app.status = 'Event names must be V/QML identifiers.'
				return false
			}
...
fn (mut app IdeApp) set_form_property(field string, value string) bool {
	match field {
		'form_property_name' {
			if !valid_identifier(value) {
				app.status = 'Form name must be a V/QML identifier.'
				return false
			}
...
fn qml_escape(value string) string {
	return value.replace('\\', '\\\\').replace('"', '\\"').replace('\n', '\\n').replace('\t', '\\t')
}

fn component_qml(component DesignerComponent) string {
	...
	match component.kind {
		'label' {
			properties << 'text: "${qml_escape(component.text)}"'
		...
		'text_field' {
			properties << 'placeholder: "${qml_escape(component.text)}"'
			properties << 'background: ${color_hex(component.background)}'
			properties << 'color: ${color_hex(component.color)}'
			properties << 'font_size: ${component.font_size:g}'
		...
		'image' {
			if component.text.len > 0 {
				properties << 'source: "${qml_escape(component.text)}"'
			}
		}
		else {}
	}
...
fn generate_qml(app &IdeApp) string {
	mut lines := [
...
fn (mut app IdeApp) sync_source() {
	app.source_text = generate_qml(app)
	app.source_modified = false
}

fn node_number(node &ui2.QNode, key string, fallback f64) !f64 {
	raw := node.prop(key)
	if raw.len == 0 {
		return fallback
	}
...
fn node_color(node &ui2.QNode, key string, fallback u32) !u32 {
	raw := node.prop(key)
	if raw.len == 0 {
		return fallback
	}
...
fn component_from_node(node &ui2.QNode, id int) !DesignerComponent {
...
fn document_from_qml(source string) !IdeSnapshot {
	root := ui2.parse_qml(source)!
	if root.tag != 'Screen' {
...
fn (mut app IdeApp) apply_source(source string) ! {
	document := document_from_qml(source)!
	app.checkpoint()
...
fn resolve_qml_path(raw_path string, root string) !string {
	trimmed := raw_path.trim_space()
	if trimmed.len == 0 {
		return error('enter a .qml file path')
	}
	...
	if os.is_dir(path) {
		...
		names.sort()
		for name in names {
			if os.file_ext(name).to_lower() == '.qml' {
				return os.real_path(os.join_path(path, name))
			}
		}
		return error('no .qml files found in `${path}`')
	}
...
fn (mut app IdeApp) open_document(raw_path string) ! {
	path := resolve_qml_path(raw_path, app.project_root)!
...
fn (mut app IdeApp) save_document() !string {
	path := resolve_qml_path(app.path_input, app.project_root)!
...
fn companion_main_source(app &IdeApp, qml_name string) string {
	const_name := 'form_source'
	return "module main\n\nimport ui2\n\nconst ${const_name} = \$embed_file('${qml_escape(qml_name)}').to_string()\n\nfn build_form() ui2.Element {\n\treturn ui2.element_from_qml(${const_name}, ui2.bounds()) or {\n\t\teprintln('Could not load ${qml_escape(qml_name)}: \${err}')\n\t\tui2.screen(0x${app.form_background:06x}, [])\n\t}\n}\n\nfn handle_form_event(event string) {\n\tprintln('event: \${event}')\n}\n\nfn main() {\n\tui2.run_window('${qml_escape(app.form_name)}', ${int(app.form_width)}, ${int(app.form_height)}, build_form, handle_form_event)\n}\n"
}

fn (mut app IdeApp) generate_companion(overwrite bool) !string {
	qml_path := if app.file_path.len > 0 { app.file_path } else { app.save_document()! }
	main_path := os.join_path(os.dir(qml_path), 'main.v')
	if os.exists(main_path) && !overwrite {
		return error('`${main_path}` already exists; it was left unchanged')
	}
	os.write_file(main_path, companion_main_source(app, os.file_name(qml_path)))!
...
fn (mut app IdeApp) new_document() {
	...
	app.file_path = ''
	app.path_input = os.join_path(app.project_root, 'form.qml')
	app.active_tab = 'designer'
	app.armed_kind = ''
	app.source_modified = false
	app.undo_stack = []IdeSnapshot{}
